...
class FEMSolver:
    ...
    def solve(self):
        ...
        self.leftbc.apply(nodes, f_array, A, self.ode, phi if quadratic == 0 else sigma, len(size)-1)
        self.rightbc.apply(nodes, f_array, A, self.ode, phi if quadratic == 0 else sigma, len(size)-1)
    ...
    def plot_solution(self):
        ...
        def u_h(x):
                ...
                for index, i in enumerate(size):
                    approx += U[index]*phi(i,x,nodes)
                if quadratic != 0:
                    for i in range(1,self.N+2):
                        approx += U[len(size)+i-1]*psi(i,x,nodes)
                if isinstance(self.leftbc, LeftDirichletBC):
                    approx += self.leftbc.leftbc*phi(0,x,nodes)
                if isinstance(self.rightbc, RightDirichletBC):
                    approx += self.rightbc.rightbc*phi(self.N+1,x,nodes)
                return approx
